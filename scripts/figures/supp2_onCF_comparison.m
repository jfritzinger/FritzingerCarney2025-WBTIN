function supp2_onCF_comparison(save_fig)
% supp_onCF_comparison plots Figure S5.
%	This function plots on-CF scatter plot comparison for WB and NB TIN,
%	for the contralateral condition. 
%
% Inputs:
...
figure('Name', 'On-CF WB NB Comparison', 'Position', [50,50,6.9*ppi,2.6*ppi]); 
...
for ibin = 1
	for iNo = 1:3 % For each No
		...
		for isesh = 1:length(index)
			...
			% Error, using propogation of uncertainty
			std_nb = (sqrt(data_NB.rate_std_onCF(3)^2 + ...
				data_NB.rate_std_onCF(1)^2))/sqrt(params_NB{1}.nrep); 
			std_wb = (sqrt(data_WB.rate_std_onCF(3)^2 + ...
				data_WB.rate_std_onCF(1)^2))/sqrt(params_WB{1}.nrep);
		...
		h(subplot_numbers(1, iNo)) = subplot(6, 3, subplot_numbers(1, iNo)); 
		...
		for iMTF = 1:4
			...
			hold(h(a), "on")
			scatter(h(a), nbwb_data(:,1), nbwb_data(:,2), 2, 'filled', ...
			...
			if iMTF == 1 || iMTF == 2
				...
				plot(h(a), e(1,:), e(2,:), 'Color',MTF_colors{iMTF}, ...
					'linewidth', 1);
			end
			...
			hold(h(b), "on")
			xline(h(b), 0, 'k')
			histogram(h(b), nbwb_data(:, 1),edges, 'Orientation', ...
				'vertical', 'EdgeColor', 'k', 'FaceColor', color);
			
			% Create distribution plot below the scatter plot (vertical)
			c = subplot_numbers(1, iNo)+2;
			hold(h(c), "on")
			yline(h(c), 0, 'k')
			histogram(h(c), nbwb_data(:, 2), edges,  'Orientation', ...
				'horizontal', 'EdgeColor', 'k', 'FaceColor', color);
		end

		% Plot 1: Figure properties
		xlim(h(a), 60*[-1 1])
		ylim(h(a), 60*[-1 1])
		yline(h(a), 0, 'k')
		xline(h(a), 0, 'k')
		xticklabels(h(a), [])
		yticklabels(h(a), [])
		title(h(a), sprintf('N_0 = %s dB SPL  ', Nos{iNo}), 'fontsize',...
			titlesize)
		if iNo == 1
			hLegend = legend(h(a), 'BE MTF','','', 'BS MTF', '','',...
				'H_B_E', '','H_B_S', 'fontsize',...
...
if save_fig == 1
	saveFigure('Supp2_onCF_comparison')
end
end
