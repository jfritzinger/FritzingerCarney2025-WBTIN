 function model_output = modelLateralSFIE_BMF(stim_params, model_params,...
	 an_sout, an_sout_lo, an_sout_hi, args)
% Runs the broad inhibition model with an option for assigning each neuron
% a different BMF. 
...
arguments
	...
	args.BMFs = [100 100 100]; % low, on, high BMF values 
end
...
tau_ex_ic = 1./(10*args.BMFs); 	% Time constant excitation in seconds
...
ic_delay_inh_array_lo = zeros(1,floor(Fs*ic_delay_inh(1)));
ic_delay_inh_array_on = zeros(1,floor(Fs*ic_delay_inh(2)));
ic_delay_inh_array_hi = zeros(1,floor(Fs*ic_delay_inh(3)));
...
for j = 1:numreps %for multiple runs
	...
	for n = 1:stim_params.num_stim
		...
		cn_ex = [cn_ex_vals cn_delay_array ic_delay_inh_array_on  ic_delay_offCF_array ic_delay_bs_array];
		cn_inh = [cn_delay_array  cn_inh_vals ic_delay_inh_array_on ic_delay_offCF_array ic_delay_bs_array];
		...
		cn_ex = [cn_ex_vals cn_delay_array ic_delay_inh_array_lo  ic_delay_offCF_array ic_delay_bs_array];
		cn_inh = [cn_delay_array  cn_inh_vals ic_delay_inh_array_lo ic_delay_offCF_array ic_delay_bs_array];
		...
		cn_ex = [cn_ex_vals cn_delay_array ic_delay_inh_array_hi  ic_delay_offCF_array ic_delay_bs_array];
		cn_inh = [cn_delay_array  cn_inh_vals ic_delay_inh_array_hi ic_delay_offCF_array ic_delay_bs_array];
		...
		for iBMF = 1:3
			[B3(iBMF, :), A3(iBMF,:)] = get_alpha_norm(tau_ex_ic(iBMF), Fs, 1);
			[B4(iBMF,:), A4(iBMF,:)] = get_alpha_norm(tau_inh_ic(iBMF), Fs, 1);
		end
		%plotAlphaFunctions(A3, B3, A4, B4, Fs, 'BE')

		% Calculate on-CF BE cell
		ic_ex_on_vals = afamp_ic*(1/Fs)*(filter(B3(2,:), A3(2,:), cn_sout));
		ic_inh_on_vals = afamp_ic*inh_str_ic*(1/Fs)*(filter(B4(2,:), A4(2,:), cn_sout));
		ic_ex_on = ic_ex_on_vals;
		ic_inh_on = [ic_delay_inh_array_on ic_inh_on_vals(1:end-length(ic_delay_inh_array_on))];
		...
		ic_ex_hi_vals = afamp_ic*(1/Fs)*(filter(B3(3,:), A3(3,:), cn_sout_hi));
		ic_inh_hi_vals = afamp_ic*inh_str_ic*(1/Fs)*(filter(B4(3,:), A4(3,:), cn_sout_hi));
		ic_ex_hi = ic_ex_hi_vals;
		ic_inh_hi = [ic_delay_inh_array_hi ic_inh_hi_vals(1:end-length(ic_delay_inh_array_hi))];
		...
		% Calculate delayed low-CF BE cell 
		ic_ex_lo_vals = afamp_ic*(1/Fs)*(filter(B3(1,:), A3(1,:), cn_sout_lo));
		ic_inh_lo_vals = afamp_ic*inh_str_ic*(1/Fs)*(filter(B4(1,:), A4(1,:), cn_sout_lo));
		ic_ex_lo = ic_ex_lo_vals;
		ic_inh_lo = [ic_delay_inh_array_lo ic_inh_lo_vals(1:end-length(ic_delay_inh_array_lo))];
		...
		for iBMF = 1:3
			[B5(iBMF, :), A5(iBMF, :)] = get_alpha_norm(tau_inh_bs(iBMF), Fs, 1);
		end
		%plotAlphaFunctions(A3, B3, A5, B5, Fs, 'BS')

		% Calculate on-CF BS cell 
		ic_bs_inh_vals = Aex*inh_str_bs*(1/Fs)*(filter(B5(2,:), A5(2,:),ic_on_BE));
		...
		ic_bs_inh_vals = Aex*inh_str_bs*(1/Fs)*(filter(B5(3,:), A5(3,:),ic_hi_BE));
		...
		ic_bs_inh_vals = Aex*inh_str_bs*(1/Fs)*(filter(B5(2,:), A5(2,:),ic_lo_BE));
		...
		ic_inh_hi_vals = afamp_ic*str_inh_hi*(1/Fs)*(filter(B4(3,:), A4(3,:), input_inh_hi));
		ic_inh_lo_vals = afamp_ic*str_inh_lo*(1/Fs)*(filter(B4(1,:), A4(1,:), input_inh_lo));
		...
		t_on = min([length(ic_ex_on) length(ic_inh_hi) length(ic_inh_lo)]);
		switch config_type
			case 'BE inhibited by off-CF CN' % BE - CN_low - CN_high
				ic = ic_ex_on(1:t_on) - ic_inh_on(1:t_on) - ic_inh_hi(1:t_on) - ic_inh_lo(1:t_on);
			case 'BS inhibited by off-CF CN' % BS - CN_low - CN_high 
				ic = ic_bs_ex(1:t_on) - ic_bs_inh(1:t_on) - ic_inh_hi(1:t_on) - ic_inh_lo(1:t_on);
			...
			case 'BS inhibited by off-CF BS' % BS - BS_low - BS_high
				ic = ic_bs_ex(1:t_on) - ic_bs_inh(1:t_on) - ic_inh_hi(1:t_on) - ic_inh_lo(1:t_on);
				%plotIntermediateConfigurations(stim_params,n, ic_bs_ex,ic_bs_inh,...
				%	ic_inh_lo,ic_inh_hi,ic, Fs, ic_bs_ex_hi, ic_bs_inh_hi)
