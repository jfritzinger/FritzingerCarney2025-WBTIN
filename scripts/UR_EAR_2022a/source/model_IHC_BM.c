...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	/* int    nrep, pxbins, lp, outsize[2], bmoutsize[2], totalstim, species;*/ /* LHC */
	int    nrep, pxbins, lp, totalstim, species; /* LHC */
	mwSize outsize[2], bmoutsize[2]; /* DMS, 27 April 2021 */

	double *pxtmp, *cftmp, *nreptmp, *tdrestmp, *reptimetmp, *cohctmp, *cihctmp, *speciestmp;
    double *ihcout, *bmout;
   
	void   IHCAN(double *, double, int, double, int, double, double, int, double *, double *); /* added another double * for bmout LHC */
	...
	if (nlhs != 2)
	{
		mexErrMsgTxt("model_IHC requires 2 output arguments."); /* LHC */
	}
    ...
    bmoutsize[0] = 1;/* LHC */
    bmoutsize[1] = totalstim;/* LHC */
    
	plhs[0] = mxCreateNumericArray(2, outsize, mxDOUBLE_CLASS, mxREAL);
	plhs[1] = mxCreateNumericArray(2, bmoutsize, mxDOUBLE_CLASS, mxREAL); /* LHC */
	
	/* Assign pointers to the outputs */
	
	ihcout  = mxGetPr(plhs[0]);
	bmout   = mxGetPr(plhs[1]); /* LHC */
		
	/* run the model */

	IHCAN(px,cf,nrep,tdres,totalstim,cohc,cihc,species,ihcout,bmout);
...
void IHCAN(double *px, double cf, int nrep, double tdres, int totalstim,
                double cohc, double cihc, int species, double *ihcout, double *bmout)  /* added bmout  LHC */
{	
