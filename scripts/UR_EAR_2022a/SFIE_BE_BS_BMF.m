...
% Vector lengths.
N_ic_delay_inh = round(ic_delay_inh*fs);
N_ic_delay_bs = round(ic_delay_bs*fs);

% Maximum value of N_ic_delay_inh when BMF = 125/4 (its smallest value), and
% zero vector to be appended to achieve the maximum length.  This is done so
% this function always returns vectors of the same length for all BMF values.
BMF_min = 125/4;
tau_ex_ic_max = 1/(10*BMF_min);
ic_delay_inh_max = tau_ex_ic_max*2;
N_ic_delay_inh_max = round(ic_delay_inh_max*fs);
extra_z = zeros(1,N_ic_delay_inh_max - N_ic_delay_inh);
...
cn_ex = [(afamp_cn/fs)*filter(B1,A1,an_sout),zeros(1,fs*cn_delay)];
cn_inh = [zeros(1,fs*cn_delay),(afamp_cn*inh_str_cn/fs)*filter(B2,A2,an_sout)];

% final CN model response:
% cn_sout = ((cn_ex-cn_inh) + abs(cn_ex-cn_inh))/2;   % subtract inhibition from excitation and half-wave-rectify
cn_sout = max(cn_ex - cn_inh,0);   % subtract inhibition from excitation and half-wave-rectify
...
ic_lp_ex1 = [(afamp_ic/fs)*filter(B3,A3,cn_sout),zeros(1,N_ic_delay_inh),extra_z];
ic_lp_inh1 = [zeros(1,N_ic_delay_inh),(afamp_ic*inh_str_ic/fs)*filter(B4,A4,cn_sout),extra_z];
% ic_sout_BE = ((ic_lp_ex1-ic_lp_inh1) + abs(ic_lp_ex1-ic_lp_inh1))/2; % half-wave rectified; standard SFIE model
ic_sout_BE = max(ic_lp_ex1 - ic_lp_inh1,0); % half-wave rectified; standard SFIE model

%  Band-suppressed cell (see Carney et al., 2015)
[B5,A5] = get_alpha_norm(tau_inh_bs,fs,1);
ic_bs_ex = Aex * [ic_lp_ex1,zeros(1,N_ic_delay_bs)]; % add zeros at end to match lengths  
ic_bs_inh = [zeros(1,N_ic_delay_bs),(Aex*inh_str_bs/fs)*filter(B5,A5,ic_sout_BE)];
% ic_sout_BS = ((ic_bs_ex-ic_bs_inh) + abs(ic_bs_ex-ic_bs_inh))/2; % half-wave rectified
ic_sout_BS = max(ic_bs_ex - ic_bs_inh,0); % half-wave rectified
