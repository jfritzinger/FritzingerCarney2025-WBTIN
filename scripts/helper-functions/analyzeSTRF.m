...
% Calculate spectrotemporal receptive field (STRF) 
%   data = analyzeSTRF(param) computes STRF using 2nd order Wiener 
%	kernel analysis, Lewis et al. (2001)
%
%   Inputs:
%       param - Structure containing experimental parameters and clustered
%				data for STRF
%
%   Outputs:
%       data - Structure containing STRF analysis results:
%           .t            - Time vector (seconds)
%           .f            - Frequency vector (Hz)
%           .H2ex_strf    - Excitatory STRF component (frequency × time)
%           .H2in_strf    - Inhibitory STRF component (frequency × time)
%           .clims_strf   - Color limits for STRF visualization
%           .tlims        - Time axis limits [min max]
%           .flims        - Frequency axis limits [min max]
%           .strf         - Net STRF (H2ex - H2in)
%           .U            - Singular vectors from H2 decomposition
%           .S            - Singular values matrix
%           .V            - Singular vectors matrix
%           .H0           - Mean firing rate component
%           .H1           - Linear kernel (time domain)
%           .H2           - Quadratic kernel (time domain)
%           .T_pts        - Number of time points in analysis window
...
[noises,~,~] = unique(double([param.list.ind]).'); % noise IDs for reproducible noises
